...
extract_landuse_FAG <- function(grid, lu_raster) {
  ...
  land_use_proportions <- land_use_counts %>%
    ...
    dplyr::select(cell50x50, value, proportion)
  ...
  land_use_rel <- land_use_proportions %>%
    dplyr::select(-value) %>% # Remove land use category numbers
    pivot_wider(names_from = LABEL3, values_from = proportion, values_fill = 0)

  land_use_rel <- land_use_rel %>%
    rowwise() %>%
    mutate(
      forest_rel = sum(c_across(all_of(forest_classes)), na.rm = TRUE),
    ...
    ungroup() %>%
    dplyr::select(cell50x50, forest_rel, agriculture_rel, grassland_rel, urban_rel, other_rel)
  ...
  land_use_proportions <- land_use_proportions %>%
    left_join(grid %>% st_drop_geometry() %>% dplyr::select(cell50x50, area_km2), by = "cell50x50")
  ...
  land_use_areas_wide <- land_use_proportions %>%
    dplyr::select(cell50x50, LABEL3, area_km2) %>%
  ...
  land_use_abs <- land_use_areas_wide %>%
    ...
    ungroup() %>%
    dplyr::select(cell50x50, forest_abs, agriculture_abs, grassland_abs, urban_abs, other_abs)
...
land_use_fa_2000 <- extract_landuse_FAG(grid, land_use_raster_2000)
head(land_use_fa_2000)


write_csv(land_use_fa_2000, "own datasets/LU_2000_FAG_50km.csv")
...
land_use_fa_2018 <- extract_landuse_FAG(grid, land_use_raster_2018)
head(land_use_fa_2018)


write_csv(land_use_fa_2018, "own datasets/LU_2018_FAG_50km.csv")
...
# ___________________________________________________________________________
## Simplify LU raster tif with 5/3/2 classes ####

simplify_land_use <- function(raster_path, output_path, class_definitions, class_names = NULL) {
  # Load raster
  r <- rast(raster_path)

  # Create lookup table
  lookup <- data.frame(
    LABEL3 = unlist(class_definitions),
    new_class = rep(seq_along(class_definitions), times = sapply(class_definitions, length))
  )

  # Get original category table
  cats_table <- cats(r)[[1]]
  ...
  # Optional: Apply custom class names
  if (!is.null(class_names)) {
    levels(r_simplified)[[1]]$label <- class_names
  }

  # Save and plot
  writeRaster(r_simplified, output_path, overwrite = TRUE)
  plot(r_simplified, main = "Simplified Land Use")
...
class_definitions_5 <- list(
  ...
  other = c(
    "Road and rail networks and associated land", "Mineral extraction sites", "Dump sites",
...
# 2000
simplify_land_use(
  raster_path = "external datasets/CORINE Land Cover CLC/u2006_clc2000_v2020_20u1_raster100m/DATA/U2006_CLC2000_V2020_20u1.tif",
  output_path = "own datasets/simplified_land_use_2000_FAUGO.tif",
  class_definitions = class_definitions_5,
  class_names = c("Forest", "Agriculture", "Urban", "Grassland", "Other")
)


# 2018
simplify_land_use(
  raster_path = "external datasets/CORINE Land Cover CLC/u2018_clc2018_v2020_20u1_raster100m/DATA/U2018_CLC2018_V2020_20u1.tif",
  output_path = "own datasets/simplified_land_use_2018_FAUGO.tif",
  class_definitions = class_definitions_5,
  class_names = c("Forest", "Agriculture", "Urban", "Grassland", "Other")
)
...
class_definitions_3 <- list(
  forest = class_definitions_5$forest,
  agriculture = class_definitions_5$agriculture,
  other = c(
    class_definitions_5$urban,
    class_definitions_5$grassland,
    class_definitions_5$other
  )
)
...
simplify_land_use(
  raster_path = "external datasets/CORINE Land Cover CLC/u2006_clc2000_v2020_20u1_raster100m/DATA/U2006_CLC2000_V2020_20u1.tif",
  output_path = "own datasets/simplified_land_use_2000_FAO.tif",
  class_definitions = class_definitions_3,
  class_names = c("Forest", "Agriculture", "Other")
)
...
simplify_land_use(
  raster_path = "external datasets/CORINE Land Cover CLC/u2018_clc2018_v2020_20u1_raster100m/DATA/U2018_CLC2018_V2020_20u1.tif",
  output_path = "own datasets/simplified_land_use_2018_FAO.tif",
  class_definitions = class_definitions_3,
  class_names = c("Forest", "Agriculture", "Other")
)
...
simplify_land_use(
  raster_path = "external datasets/CORINE Land Cover CLC/u2006_clc2000_v2020_20u1_raster100m/DATA/U2006_CLC2000_V2020_20u1.tif",
  output_path = "own datasets/simplified_land_use_2000_NHD.tif",
  class_definitions = class_definitions_2,
  class_names = c("Natural", "Human_Dominated")
)
...
simplify_land_use(
  raster_path = "external datasets/CORINE Land Cover CLC/u2018_clc2018_v2020_20u1_raster100m/DATA/U2018_CLC2018_V2020_20u1.tif",
  output_path = "own datasets/simplified_land_use_2018_NHD.tif",
  class_definitions = class_definitions_2,
  class_names = c("Natural", "Human_Dominated")
)
...
write_csv(centroids_df, "own datasets/ebba_grid_50km_centroids.csv")
write_csv(centroids_df_change, "own datasets/ebba_change_grid_50km_centroids.csv")
...
ebba1_matrix <- species_matrix(ebba1, coords_df = centroids_df)
ebba2_matrix <- species_matrix(ebba2, coords_df = centroids_df)

ebba1_matrix_change <- ebba_change(ebba1_matrix, location)
ebba2_matrix_change <- ebba_change(ebba2_matrix, location)



# __________________________________________________________________________________________________

write_csv(ebba1_matrix_change, "own datasets/ebba1_matrix_centroids.csv")
write_csv(ebba2_matrix_change, "own datasets/ebba2_matrix_centroids.csv")
...
lu2000 <- read_csv("own datasets/LU_2000_FAG_50km.csv")
...
# Load datasets
land_use_2000 <- read_csv("own datasets/LU_2000_FAG_50km.csv")
land_use_2018 <- read_csv("own datasets/LU_2018_FAG_50km.csv")
...
SR_specializations_FAG <- read_csv("own datasets/SR_classes_FAG.csv")
SR_specializations_NHD <- read_csv("own datasets/SR_classes_NHD.csv")
...
write_csv(df_cSAR_FAG_b, "own datasets/cSAR_FAG_b_2000.csv")
...
write_csv(df_cSAR_FAG_b, "own datasets/cSAR_FAG_b_2018.csv")
...
# df_cSAR_FAG_b = read_csv("own datasets/cSAR_FAG_b.csv")

stpr <- matrix(
  c(
    1.000e+00, 1.237e-03, 1.984e-03, 1.006e-03, 1.572e-03, 1.534e-01,
...
cSAR_FAG_b <- sar_countryside(
  data = df_cSAR_FAG_b, modType = "power",
...
df_cSAR_NHD_b <- df_cSAR_NHD1 %>%
  dplyr::select(
    cell50x50, forest, agriculture, grassland, urban, other,
...
write_csv(df_cSAR_NHD_b, "own datasets/cSAR_NHD_b.csv", row.names = TRUE)

# df_cSAR_NHD_b = read_csv("own datasets/cSAR_NHD_b.csv")


cSAR_NHD_b <- sar_countryside(
  data = df_cSAR_NHD_b, modType = "power",
  startPar = stpr,
  habNam = c("F", "A", "G", "U", "O"),
  spNam = c("F_Sp", "A_Sp", "G_Sp")
)
...
df_cSAR_FAG_m <- df_cSAR_FAG1 %>%
  dplyr::select(
    cell50x50, forest, agriculture, urban, other,
  ...
  dplyr::select(cell50x50, Area_F, Area_A, Area_O, Spcs_F, Spcs_A, Spcs_O) %>%
  column_to_rownames("cell50x50")


str(df_cSAR_FAG_m)

cSAR_FAG_m <- sar_countryside(
  data = df_cSAR_FAG_m, modType = "power",
...
# Load datasets
land_use_fa_2000 <- read_csv("own datasets/LU_2000_FAG_50km.csv")
land_use_fa_2018 <- read_csv("own datasets/LU_2018_FAG_50km.csv")
...
write_csv(change, "own datasets/LU_change_50km.csv")
...
long_df <- change %>%
  dplyr::select(cell50x50, forest, agriculture) %>%
  pivot_longer(
    cols = c(forest, agriculture),
    names_to = "Category",
...
ggsave("plots/land use/boxplot_rel_forest_agric_area_change.png", plot = plot, width = 7, height = 8, dpi = 300, bg = "white")


png("plots/land use/hist_rel_forest_area_change.png",
  width = 800,
  height = 600,
  res = 100
)
...
png("plots/land use/hist_rel_agric_area_change.png",
  width = 800,
  height = 600,
  res = 100
)
...
ggsave("plots/land use/LU_change_forest_50km.png", plot = plot, width = 10, height = 8, dpi = 300, bg = "white")
...
ggsave("plots/land use/LU_change_agric_50km.png", plot = plot, width = 10, height = 8, dpi = 300, bg = "white")
...
data <- read_csv("own datasets/analysis_data_50km.csv")
