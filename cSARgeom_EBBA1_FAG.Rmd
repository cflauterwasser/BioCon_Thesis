---
title: "cSAR geometry EBBA1 (Forest/Agriculture/Other)"
...
summarize_samples <- function(samples, polygons, habitat_raster, habitat_names,
  ...
  results_df <- data.frame(matrix(
    ncol = length(habitat_names) +
...
```{r}
class(points)
squares_sf <- create_squares(points, width = 50000)
...
for (size in seq_along(clusters_chulls))
{
  ...
  plot(st_geometry(points), col = rgb(0, 1, 0, 0))
...
classif <- read.csv("own datasets/aves_classification_EBBA1_fag.csv")
...
res <- summarize_samples(
  flatten(samples), flatten(polygons),
...
datacsar <- res[, c(-6, -10)]
species_group_names <- colnames(datacsar[, 6:8])

s3 <- sar_countryside(
  data = datacsar, modType = "power",
  gridStart = "none",
  habNam = c("Forest", "Agriculture", "Urban", "Grassland", "Other"), # habitat_names,
  spNam = c("forest_sp", "agriculture_sp", "generalist_sp")
) # species_group_names)
s3
```
